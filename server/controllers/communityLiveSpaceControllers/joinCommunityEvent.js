...
const joinCommunityEvent = async (req, res) => {
    try {
        ...
        const liveSpace = await communityLiveSpaceModel.findById(eventId);
        ...
        return res.status(200).json({
            success: true,
            message: "User has successfully joined the community live space",
            liveSpace: liveSpace,
        });
    } catch (error) {
        ...
        return res.status(500).json({
            success: false,
            message: "Error while joining the community live space",
        });
    }
};

export default joinCommunityEvent;