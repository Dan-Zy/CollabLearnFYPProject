...
function CommunityEvents({ communityId }) {
    ...
    const fetchEvents = async (token) => {
        try {
            ...
            const sortedEvents = resp.data.Events.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    ...
    // Event handler for joining an event
    const handleJoinEvent = async (eventId) => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.put(
                `http://localhost:3001/collablearn/joinCommunityEvent/${eventId}`,
                {},
                {
                    headers: {
                        Authorization: `${token}`
            ...
            {isAdmin && (
                <div className="flex flex-col w-full bg-white shadow-md rounded-lg m-1 p-4 lg:flex-2 sm:flex-1 text-[1vw] min-h-[200px]">
                    ...
                    <button
                        ...
                        className={`flex justify-center text-center text-white ${loading ? 'bg-gray-400' : 'bg-indigo-500 hover:bg-indigo-600'} text-[1vw] p-2 rounded`}
                    >
                        {loading ? 'Creating...' : 'GoLive'}
                    </button>
                </div>
            )}
                ...
                {events.length > 0 ? (
                    events.map((event) => (
                        <div key={event._id} className="flex flex-col m-4 items-center justify-center bg-white shadow-lg rounded-lg">
                            ...
                            <button
                                onClick={() => handleJoinEvent(event._id)} // Call the handler here
